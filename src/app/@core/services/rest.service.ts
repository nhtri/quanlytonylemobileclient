...
export default class RestService {
    ...
    public search<T>(resourceUri?: string, queryPrams?: Record<string, unknown>, headers?: {
        [header: string]: string | string[];
    }): Observable<T> {
        return this.post(resourceUri, queryPrams, headers);
    }

    // For custom request use
    public post<T>(resourceUri?: string, data?: any | undefined, headers?: {
        [header: string]: string | string[];
    }): Observable<T> {
        if (notEmpty(headers)) {
            this.httpOptions.headers = new HttpHeaders(headers);
