...
import {PRODUCT_SOURCE} from '../../../constant/common';
...
* 1. Currency Mask had to be easy to implement across application.
...
*    On Edit mode the the indicative of negative number is the minus "-" sign, but when
...
export class CurrencyMaskDirective implements AfterViewInit, ControlValueAccessor {
    ...
    @Input('shopSource')
    shopSource: PRODUCT_SOURCE = PRODUCT_SOURCE.SHOP_JP;
    ...
    writeValue(value: any) {
        if (value !== this.innerValue) {
            this.el.value = this.currencyMaskService.transform(value, this.shopSource, this.allowNegative);
            if (value) {
