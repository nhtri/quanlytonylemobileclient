...
export class QuanlythunhatComponent implements OnInit {
  ...
  constructor(private service: NetworkserviceService) {
    ...
    this.service.getquanlythujpdanhsachdonhang().subscribe(val => {
      console.log(val)
      val.forEach(element => {
        this.totalmoney += Number(element.sotien)
        if (element.mucdich.includes('dh')) {
  ...
  hoantat() {
    ...
    if (this.id == "") {
      ...
      this.service.quanlythu([this.sotien, this.date, this.mucdich, this.hinhthucthanhtoan, 'SHOP_JP']).subscribe(val => {
